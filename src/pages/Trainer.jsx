...
import { levelKey, levelDescKey, phaseKey } from '../lib/keys'
...
function Trainer() {
  ...
  const getLevelTitle = (level) => {
    const key = levelKey(level.id)
    const hit = t(key)
    return hit !== key ? hit : level.id
  }

  const getLevelDesc = (level) => {
    const key = levelDescKey(level.id)
    const hit = t(key)
    return hit !== key ? hit : ''
  }

  const getLevelFocus = (level) => level.focus

  const getPhaseTitle = (phase) => {
    if (!phase) return ''
    const key = phaseKey(phase.id)
    const hit = t(key)
    return hit !== key ? hit : phase.id
  }
  ...
  const handleInput = (e) => {
    ...
    const started = !startTime && val.length > 0 ? Date.now() : startTime
    ...
    if (val === targetText && targetText.length > 0) {
      setIsComplete(true)
    ...
    const stats = computeStats({ input: val, target: targetText, startTime: started })
    setWpm(stats.wpm)
  ...
  return (
    <div className="unified-app">
      ...
      <Navbar variant="trainer" lang={lang} onToggleLang={toggleLang} />
              ...
              <div key={phase.id} className="phase-section" style={{ '--phase-color': phase.color }}>
                ...
                <div className="phase-levels">
                  {phase.levels.map((level, i) => {
                    const phaseIndex = curriculum.phases.indexOf(phase)
                    const globalIndex = phaseIndex === 0
                      ? i
                      : curriculum.phases.slice(0, phaseIndex).reduce((acc, p) => acc + p.levels.length, 0) + i
                    return (
                      <button
                        key={level.id}
